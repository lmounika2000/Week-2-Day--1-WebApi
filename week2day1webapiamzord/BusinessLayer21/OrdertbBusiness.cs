...
namespace BusinessLayer21
{
    public class OrdertbBusiness : IOrdertbBusiness
    {
        ...
        public async Task<List<Orderstb>> GetAllOrdersByCountryName(string name)
        {
            return await _ordertbdata.GetAllOrdersByCountryName(name);
        }

         public async Task<CountryOrdersSum> SumOfOrdersCostByCountryName(string name)
        {
            var orders = await _ordertbdata.SumOfOrdersCostByCountryName(name);
            var TotalCost = orders.Sum(x => x.Cost);

            CountryOrdersSum cos = new CountryOrdersSum();
            cos.TotalCost = TotalCost;
